...
for (let pg=0; pg<4; pg++){
  await p.evaluate((n)=>{ window.DSi.settings.page=n; window.DSi.settings.sel=0; }, pg);
  await p.waitForTimeout(120);
  for (const sc of ['top','bottom']){
    const d=await p.evaluate((id)=>document.getElementById(id).toDataURL('image/png'), sc);
    fs.writeFileSync(`/work/nds/compare/settings/pg${pg+1}.${sc}.png`, Buffer.from(d.split(',')[1],'base64'));
  }
}
await b.close(); s.close(); console.log('captured settings');
