...
await p.evaluate(() => {
  ...
  st.confirmExit = { t: 12, sel: -1 };  // fully open, no dpad highlight (matches g_0044)
});
...
for (const sc of ['top','bottom']) {
  ...
  fs.writeFileSync(`/work/nds/compare/settings/dlg.${sc}.png`, Buffer.from(d.split(',')[1],'base64'));
}
await b.close(); s.close(); console.log('captured dlg');
