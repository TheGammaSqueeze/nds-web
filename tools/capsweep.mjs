...
await p.evaluate(()=>{ window.__freezeAnim=120; DSi.skipBoot(); DSi.launcher.intro=null; });
for (const s of [0,8,16,24,32,38]) {
  ...
  fs.writeFileSync(`compare/zoom/cs_${String(s).padStart(2,'0')}.png`, Buffer.from(url.split(',')[1],'base64'));
}
